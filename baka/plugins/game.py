...
async def kill(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"💰 <b>Earned:</b> <code>{format_money(final_reward)}</code>", 
...
async def rob(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not target: return await update.message.reply_text("⚠️ Tag victim", parse_mode=ParseMode.HTML)
...
async def protect(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"🛡️ <b>Shield Active!</b> Safe for {days} day(s).", parse_mode=ParseMode.HTML)
...
async def revive(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if target['status'] == 'alive': 
        return await update.message.reply_text("✨ Alive!", parse_mode=ParseMode.HTML)
...
async def pay(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try: amount = int(context.args[0])
    except: return await update.message.reply_text("⚠️ Valid amount dalo.", parse_mode=ParseMode.HTML)

    if user['balance'] < amount: 
        return await update.message.reply_text("❌ Paisa kam hai!", parse_mode=ParseMode.HTML)

    target, _ = await resolve_target(update, context)
    if not target: return await update.message.reply_text("⚠️ User tag karo.", parse_mode=ParseMode.HTML)
