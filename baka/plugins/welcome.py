    ...
    for member in update.message.new_chat_members:
        ...
        if member.id == context.bot.id:
            ...
            txt = (
                ...
                f"Type <code>/claim</code> to get 2,000 Coins!"
            )
            kb = InlineKeyboardMarkup([[InlineKeyboardButton("💬 𝐒𝐮𝐩𝐩𝐨𝐫𝐭", url=SUPPORT_GROUP)]])

            try:
                await update.message.reply_photo(WELCOME_IMG_URL, caption=txt, parse_mode=ParseMode.HTML, reply_markup=kb)
            except:
        ...
        # --- 👤 USER JOINED GROUP ---
            ...
            if group_data and group_data.get("welcome_enabled", True):
                # Ekdum minimal welcome text
                txt = f"𝐖𝐞𝐥𝐜𝐨𝐦𝐞 {get_mention(member)}! ✨"
                try:
                    await update.message.reply_photo(WELCOME_IMG_URL, caption=txt, parse_mode=ParseMode.HTML)
                except:
