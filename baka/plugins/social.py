...
# Edited for Malik: ZEXX (Fast & Clean Version)
...
async def propose(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not target: 
        return await update.message.reply_text(error or "⚠️ <b>Usage:</b> <code>/propose @user</code>", parse_mode=ParseMode.HTML)
    ...
    if target.get('partner_id'): 
        return await update.message.reply_text("💔 Taken.", parse_mode=ParseMode.HTML)
...
async def divorce(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user['balance'] < DIVORCE_COST: 
        return await update.message.reply_text(f"❌ Cost: {format_money(DIVORCE_COST)}", parse_mode=ParseMode.HTML)
...
async def proposal_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "marry_y":
        ...
        await query.message.edit_text(
            f"💍 <b>Married!</b>\n<a href='tg://user?id={p_id}'>Partner 1</a> ❤️ <a href='tg://user?id={t_id}'>Partner 2</a>", 
            parse_mode=ParseMode.HTML
        )
    elif action == "marry_n":
        # AI hata kar fast local roast add kar diya
        roasts = [
            "Auqat se bahar try mat kar baby!",
            "Rejected! Agli gali dekho.",
            "Shakal dekhi hai apni? Reject!",
            "Bechara... Friendzone ho gaya!",
            "ZEXX ke bot ne kaha: Try again in next life."
        ]
